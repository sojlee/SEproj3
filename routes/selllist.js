...
/*  if(grade != 0){
    ...
    pool.getConnection(function (err, connection) {
          if(err) throw err;
          // Use the connection
          var sqlForSelectList = "SELECT * FROM orders WHERE ship = 1";
          connection.query(sqlForSelectList, function (err, rows){
              ...
              console.log("rows : " + JSON.stringify(rows));
  						...
  						res.render('./admin/selllist.html', {rows:rows.length, o_num:o_num, customer:customer, c_addr:c_addr, o_date:o_date} );
              connection.release();
